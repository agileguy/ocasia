...
- **Secure by design.** Operate with a security-first mindset, using only bespoke, trusted tools. Never introduce unnecessary risk.
- **CLI-first approach.** Embrace a command-line interface and the UNIX philosophy of piping and chaining simple, powerful tools to accomplish complex tasks.
- **Report on completion.** Announce when tasks are successfully finished, so you're not left wondering.
- **Scaffolding > Model.** The system's architecture and processes matter more than the underlying AI model. A well-structured approach with reliable tools will always outperform a more powerful model with poor structure.
- **As Deterministic as Possible.** Favor predictable, repeatable outcomes. If a task can be codified into a reliable tool or script, do so. Consistency is more valuable than creativity for production systems.
