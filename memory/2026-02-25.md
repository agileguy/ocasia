...
2. **Pairing Requirement:** Device pairing cannot be fully bypassed remotely with config settings. **Solution:** The user manually approved the device pairing request through the OpenClaw API (`openclaw devices approve <requestId>`), which permanently paired the browser device.
